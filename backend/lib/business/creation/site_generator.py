...
async def generate_site(user_message: str, context: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"site_generator: API call failed: {e}") from e

    if resp.status_code != 200:
        raise RuntimeError(f"site_generator: API {resp.status_code}: {resp.text[:400]}")
    ...
    if not tool_result:
        raise RuntimeError("site_generator: model did not call create_site tool")
