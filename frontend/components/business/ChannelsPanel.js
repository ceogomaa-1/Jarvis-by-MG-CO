...
export default function ChannelsPanel({ userId, email }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(255,46,81,0.18)',
      background: 'rgba(255,46,81,0.03)',
      overflow: 'hidden',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            ...
            color: '#f4f4f2', textTransform: 'uppercase', marginBottom: 2,
          }}>
            Jarvis on Telegram
          </div>
      ...
      <div style={{ padding: '0 16px 14px', borderTop: '1px solid rgba(244,244,242,0.04)' }}>
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 0', color: 'rgba(244,244,242,0.4)', fontSize: 11 }}>
          ...
          <div style={{ paddingTop: 12 }}>
            <p style={{ fontSize: 11, color: 'rgba(244,244,242,0.6)', margin: '0 0 10px' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <code style={{
                    ...
                    color: '#ff2e51', background: 'rgba(255,46,81,0.08)', padding: '6px 12px', borderRadius: 8,
                  }}>{code.code}</code>
                  <button onClick={copyCode} style={btn('ghost')}>
                    {copied ? <Check size={11} /> : <Copy size={11} />} {copied ? 'Copied' : 'Copy'}
                  </button>
                </div>
                <ol style={{ fontSize: 11, color: 'rgba(244,244,242,0.55)', lineHeight: 1.7, margin: '0 0 12px', paddingLeft: 16 }}>
                ...
                <p style={{ fontSize: 10, color: 'rgba(244,244,242,0.3)', marginTop: 10 }}>
...
function Note({ children, danger }) {
  return (
    <div style={{
      ...
      background: danger ? 'rgba(239,68,68,0.07)' : 'rgba(244,244,242,0.03)',
      border: `1px solid ${danger ? 'rgba(239,68,68,0.18)' : 'rgba(244,244,242,0.08)'}`,
      fontSize: 11, color: danger ? 'rgba(239,68,68,0.85)' : 'rgba(244,244,242,0.5)', lineHeight: 1.5,
    }}>
      {children}
    </div>
  )
...
function btn(kind) {
  ...
  if (kind === 'primary') return { ...base, background: '#ff2e51', color: '#fff' }
  if (kind === 'danger') return { ...base, background: 'transparent', color: 'rgba(239,68,68,0.8)', border: '1px solid rgba(239,68,68,0.22)' }
  return { ...base, background: 'transparent', color: 'rgba(244,244,242,0.6)', border: '1px solid rgba(244,244,242,0.12)' }
}
