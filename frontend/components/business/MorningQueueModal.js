...
function ActionCard({ action, onUpdate, busy }) {
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${isExternal ? 'rgba(200,75,49,0.25)' : 'rgba(243,234,217,0.1)'}`,
      borderRadius: 12, padding: '14px 18px', marginBottom: 10,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
        <span style={{
          fontSize: 9, fontWeight: 700, letterSpacing: '0.1em',
          color: isExternal ? '#c84b31' : 'rgba(243,234,217,0.6)',
          textTransform: 'uppercase',
          background: isExternal ? 'rgba(200,75,49,0.1)' : 'rgba(243,234,217,0.06)',
          padding: '2px 6px', borderRadius: 4,
        }}>
          {isExternal ? '⚡ EXTERNAL' : '🧠 INTERNAL'}
        </span>
        <span style={{ fontSize: 10, color: 'rgba(243,234,217,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          {action.action_type?.replace(/_/g, ' ')}
        </span>
      </div>
      <div style={{ fontSize: 14, color: '#f3ead9', fontWeight: 600, marginBottom: 4 }}>
        {action.title}
      </div>
      {action.description && (
        <div style={{ fontSize: 12, color: 'rgba(243,234,217,0.6)', marginBottom: 8 }}>
          {action.description}
        </div>
      )}
      ...
      {expanded && action.artifact_markdown && (
        <div style={{
          ...
          <div className="biz-markdown" style={{ fontSize: 13, color: '#f3ead9', lineHeight: 1.65 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            background: 'rgba(243,234,217,0.04)', border: '1px solid rgba(243,234,217,0.12)',
            borderRadius: 6, padding: '6px 12px', color: '#f3ead9', fontSize: 11,
