...
export default function UsageCounter({ usage }: UsageCounterProps) {
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: isOut ? "rgba(244,63,94,0.1)" : "rgba(243,234,217,0.04)",
        border: `1px solid ${isOut ? "rgba(244,63,94,0.2)" : isLow ? "rgba(200,75,49,0.2)" : "rgba(243,234,217,0.06)"}`,
      }}
    >
      <div
        className="relative overflow-hidden rounded-full"
        style={{ width: "40px", height: "4px", backgroundColor: "rgba(243,234,217,0.1)" }}
      >
        <div
          className="absolute left-0 top-0 h-full rounded-full transition-all duration-500"
          style={{
            width: `${Math.min(percentage, 100)}%`,
            backgroundColor: isOut ? "#f43f5e" : isLow ? "#c84b31" : "rgba(243,234,217,0.4)",
          }}
        />
      </div>
      <span
