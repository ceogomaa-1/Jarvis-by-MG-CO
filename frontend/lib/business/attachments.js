...
// chat-attachments RLS policy: (storage.foldername(name))[1] = auth.uid()::text
...
// Uploads a file under {user_uuid}/{conversation_id_or_pending}/{uuid}-{filename}.
...
export async function uploadChatAttachment(file, userId, conversationId) {
  if (!supabase || !userId) return null
  const uuid = userIdToUuid(userId)
  const convPart = conversationId || 'pending'
  const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_')
  const path = `${uuid}/${convPart}/${uuidv4()}-${safeName}`
  try {
...
export function isImageAttachment(att) {
  return (att?.media_type || '').startsWith('image/') || att?.type === 'image'
}
